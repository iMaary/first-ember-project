...
const debug = debug_1.default('ember-auto-import:leader');
/*
...
class LeaderChooser {
    ...
    register(addon, create) {
        ...
        if (!semver_1.satisfies(addon.pkg.version, '>=2.0.0-alpha.0', { includePrerelease: true })) {
            // versions older than 2.0 are not eligible to lead
            return;
        }
        if (shared_internals_1.isDeepAddonInstance(addon)) {
    ...
    get leader() {
        if (!this.locked) {
            if (!this.appCandidate) {
                throw new Error(`To use these addons, your app needs ember-auto-import >= 2: ${this.addonCandidates
                    .map(c => c.parentName)
                    .sort()
                    .join(', ')}`);
            }
            let eligible = [this.appCandidate, ...this.addonCandidates].filter(c => semver_1.satisfies(c.version, this.appCandidate.range, { includePrerelease: true }));
            ...
            for (let challenger of rest) {
                if (semver_1.gt(challenger.version, candidate.version)) {
